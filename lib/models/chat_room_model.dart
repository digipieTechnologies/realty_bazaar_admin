// File: lib/models/chat_room_model.dart
// Purpose: Generic model for chat rooms in brokerflow_admin supporting support tickets and video requests.

import 'package:equatable/equatable.dart';

import 'support_ticket_model.dart';

class ChatRoomModel extends Equatable {
  final String id;
  final String? videoRequestId;
  final String? supportTicketId;
  final String brokerId;
  final DateTime createdAt;
  final DateTime updatedAt;
  final SupportTicketModel? supportTicket;
  final String? title;
  final String? roomType; // 'video_request', 'support_ticket', etc.

  const ChatRoomModel({
    required this.id,
    this.videoRequestId,
    this.supportTicketId,
    this.brokerId = '',
    required this.createdAt,
    required this.updatedAt,
    this.supportTicket,
    this.title,
    this.roomType = 'support_ticket',
  });

  factory ChatRoomModel.fromJson(dynamic json) {
    if (json is! Map) {
      return ChatRoomModel(id: '', createdAt: DateTime.now(), updatedAt: DateTime.now());
    }

    return ChatRoomModel(
      id: json['id']?.toString() ?? '',
      videoRequestId: json['video_request_id']?.toString(),
      supportTicketId: json['support_ticket_id']?.toString(),
      brokerId: json['broker_id']?.toString() ?? '',
      createdAt: json['created_at'] != null
          ? (DateTime.tryParse(json['created_at'].toString())?.toLocal() ?? DateTime.now())
          : DateTime.now(),
      updatedAt: json['updated_at'] != null
          ? (DateTime.tryParse(json['updated_at'].toString())?.toLocal() ?? DateTime.now())
          : DateTime.now(),
      supportTicket: json['support_ticket'] != null
          ? SupportTicketModel.fromJson(json['support_ticket'])
          : null,
      title: json['title']?.toString(),
      roomType: json['room_type']?.toString() ?? 'support_ticket',
    );
  }

  Map<String, dynamic> toJson() {
    return {
      'id': id,
      'video_request_id': videoRequestId,
      'support_ticket_id': supportTicketId,
      'broker_id': brokerId,
      'created_at': createdAt.toUtc().toIso8601String(),
      'updated_at': updatedAt.toUtc().toIso8601String(),
      'title': title,
      'room_type': roomType,
    };
  }

  @override
  List<Object?> get props => [
    id,
    videoRequestId,
    supportTicketId,
    brokerId,
    createdAt,
    updatedAt,
    title,
    roomType,
  ];
}
