...
# ==============================================================================
# REALTY BAZAAR SUPER ADMIN - DEPLOYMENT & BUILD CLI
# ==============================================================================
# Interactive and automated CLI to clean, build, and deploy the Realty Bazaar
# Super Admin application across Development and Production environments for Web,
# Android, and iOS.
# ==============================================================================
...
print_banner() {
    ...
    echo "                                                                      "
    echo "   R E A L T Y   B A Z A A R   S U P E R   A D M I N   -   C L I      "
    echo "======================================================================"
    echo -e "${NC}"
...
build_web_prod() {
    ...
    run_step "Web Build [PROD / ADMIN]" "$FLUTTER_CMD build web --release --dart-define-from-file=$PROD_ENV_FILE"
}
...
deploy_web_prod() {
    run_step "Firebase Deploy to Hosting [PROD / ADMIN: $PROD_HOSTING_TARGET]" \
...
build_and_deploy_web_both() {
    echo -e "\n${YELLOW}${BOLD}➤ Starting Two-Stage Deployment (DEV then ADMIN)...${NC}"
    build_and_deploy_web_dev
    build_and_deploy_web_prod
    echo -e "\n${GREEN}${BOLD}🎉 Both DEV and ADMIN Web targets deployed successfully!${NC}"
}
...
show_menu() {
    ...
    echo -e "  ${CYAN}[2]${NC} Build Web & Deploy to ${GREEN}PROD / ADMIN${NC} (target: ${GREEN}$PROD_HOSTING_TARGET${NC}, env: ${GREEN}$PROD_ENV_FILE${NC})"
    echo -e "  ${CYAN}[3]${NC} Build & Deploy Web to ${PURPLE}BOTH${NC} (Dev & Admin Sequentially)"
    echo -e "  ${CYAN}[4]${NC} Build Web Only [${YELLOW}DEV${NC}]"
    echo -e "  ${CYAN}[5]${NC} Build Web Only [${GREEN}PROD / ADMIN${NC}]"
    echo -e "  ${CYAN}[6]${NC} Deploy Web Only [${YELLOW}DEV${NC}]"
    echo -e "  ${CYAN}[7]${NC} Deploy Web Only [${GREEN}PROD / ADMIN${NC}]"
...
if [ "$1" != "" ]; then
    case "$1" in
        web-dev|--web-dev|dev|1)
            ...
            exit 0
            ;;
        web-admin|web-prod|--web-admin|--web-prod|admin|web|prod|2)
            build_and_deploy_web_prod
            exit 0
            ;;
        web-both|--web-both|3)
        ...
        build-web-dev|--build-web-dev|4)
            ...
            exit 0
            ;;
        build-web-admin|build-web-prod|--build-web-admin|--build-web-prod|5)
            build_web_prod
            exit 0
            ;;
        deploy-web-dev|--deploy-web-dev|6)
            ...
            exit 0
            ;;
        deploy-web-admin|deploy-web-prod|--deploy-web-admin|--deploy-web-prod|7)
            deploy_web_prod
            exit 0
            ;;
        ...
        targets|--targets|14)
            ...
            ;;
        --help|-h|help)
            echo "Usage: ./tool/deploy.sh [command]"
            echo ""
            echo "Commands:"
        ...
        *)
            ...
            echo -e "Run ${YELLOW}./tool/deploy.sh --help${NC} for available commands."
            exit 1
            ;;
    esac
fi
